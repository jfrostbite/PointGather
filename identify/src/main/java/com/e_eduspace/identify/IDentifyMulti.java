 ...
 * 创建公用builder。
 *
 * 断点续存 需要注意：2点确定一个loc  所以所有设计loc  counter 操作的值全部为2倍增长
 */
...
//            throw new RuntimeException("证书初始化失败！");
     ...
     *
     * 断点采集思路： 记录上一次整夜索引编号、和loc编号
     *
     * @param point 欲收集点
     */
    public void addPoint(boolean recog, PointEntity point) {
        ...
        if (TextUtils.isEmpty(tag)) {
            LogUtils.e("当前页收集完毕");
            mContext.getContentResolver().notifyChange(Constants.POINT_URI_NEW, null);
            return;
        }

        /**
         * tag改变，说迷宫loc 重置，兼容断点首次进入，判断续点
         */
        if (tag.equals(mTag)) {
//            mLoc++;
        } else {
            mTag = tag;
//            mLoc = mDefLoc;
            //首次用完该值 重置默认值为0，兼容tag切换情况 首次传入的 Defloc只用一次
//            mDefLoc = 0;
        }

        if (mCounter % 2 != 0) {//奇数保存数据
            pae.setMaxX(point.getPX())
                    .setMaxY(point.getPY());
            LogUtils.e(pae.toString());
            addPoint(pae, recog);
            //发送添加点的通知
            mContext.getContentResolver().notifyChange(Constants.POINT_URI_ADD, null);
        } else {//偶数创建数据对象
            pae = new PointAreaEntity().setTag(tag)
                    .setLoc(String.valueOf(mLoc / 2))
    ...
    private String getTagLoc(boolean recog) {
        int len = 0;
        String tag = "";
        if (recog) {
            for (FormPage.Area area : mFormPage.area) {
                ...
                if (mCounter < len) {
                    tag = area.name;
                    //计算loc，成员索引 - 当前成立（len - names.length）
                    mLoc = mCounter - (len - (names.length * 2));
                    break;
                }
            }
        } else {
            ...
            for (int i = 0; i < selects.length; i++) {
                ...
                if (mCounter < len) {
                    tag = name;
                    mLoc = mCounter - (len - (mFormPage.selectCounts[i] * 2));
                    break;
                }
            }
        }
    ...
    public PointAreaEntity queryLast(boolean regon) {
        return mManager.query(regon, String.valueOf(mPageIndex));
    }

    /**
     * 坐标点添加数据重置
     */
    public void recover (int counter) {
        this.mCounter = counter;
    }

    public void clean(boolean recog) {
        mManager.delete(recog);
        recover(0);
        mContext.getContentResolver().notifyChange(Constants.POINT_URI_ADD, null);
    }
    ...
    public void convert(final List<? extends StrokeEntity> strokes) {
        ...
        mFutures.add(mExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                if (areaCache == null || areaCache.isEmpty()) {//没检测到需要识别的数据，可以反悔结果
                    if ( mListener != null) {
