...
public class PointRangeDB extends DBManager {
    ...
    public boolean delete(boolean recog) {
        ...
        //自增长归1
        mSQLiteDatabase.execSQL("DELETE FROM sqlite_sequence WHERE name='" + (recog ? Constants.POINT_RANGE_IDENTIFY_TAB_NAME : Constants.POINT_RANGE_TAB_NAME)+"'");
        return delete > 0;
    }
    ...
    @Override
    public PointAreaEntity query(boolean recog, String pageIndex) {
        Cursor cursor = mSQLiteDatabase.query(recog ? Constants.POINT_RANGE_IDENTIFY_TAB_NAME : Constants.POINT_RANGE_TAB_NAME, new String[]{}, Constants.POINT_RANGE_PAGE + "=?", new String[]{pageIndex}, null, null, Constants.POINT_RANGE_ID + " desc", "0,1");
        ...
        if (cursor.moveToNext()) {
            ...
            pae = new PointAreaEntity().setId(id)
                    .setTag(tag)
                    .setLoc(loc)
                    .setPageIndex(page)
                    .setMinX(minX)
